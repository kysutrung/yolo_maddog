# gamepad.py
from config import AX_LX, AX_LY, AX_RY, AX_RX, DEADZONE, SWAP_REAL_LR
...
class GamepadBridge:
    ...
    def read_axes_real(self):
        """Read real joystick axes (LX, LY, RX, RY) in [-1,1]."""
        ...
        self.r_lx, self.r_ly, self.r_rx, self.r_ry = [
            max(-1, min(1, float(v))) for v in [lx, ly, rx, ry]
        ]
        return self.r_lx, self.r_ly, self.r_rx, self.r_ry

    def send_to_virtual(self, lx, ly, rx, ry):
        """Send axes to virtual Xbox gamepad."""
