# yolo_gamepad_forward_tk_spacer_enhanced_crosshair_auto_follow_navhold_distance_followrule_no_auto_rx.py
# GUI with enhanced persistent tracking + center/target crosshair
# Follow mode:
#   - Left/Right ONLY affects Yaw (LX) automatically; RX is NEVER auto-driven (manual only).
#   - Forward/Backward (RY) is distance-based: Far -> forward, Too Close -> backward, Near -> hold.
        ...
        while self.running:
            ...
            if self.target_follow and self.gp.state == ForwardState.OFF and current_target_box is not None:
                ...
                if cmd_text != self.last_nav_cmd:
                    self.log(f"NAV CMD (Yaw only) -> {cmd_text}")
